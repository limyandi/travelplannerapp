...
import com.citygrid.content.reviews.CGReviewsSearchSort;
...
public class List extends ListActivity implements AdapterView.OnItemClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CityGrid.setSimulation(simulation);
		
		if(category.equalsIgnoreCase("Places")){
			String result[] = findPlaces();
			Toast.makeText(getApplicationContext(), "Number of places found: " + result.length, Toast.LENGTH_LONG).show();
			setListAdapter(new ArrayAdapter<String>(this, R.layout.list, result));
		}
		else if(category.equalsIgnoreCase("Offers")){
			String result[] = findOffers();
			Toast.makeText(getApplicationContext(), "Number of offers found: " + result.length, Toast.LENGTH_LONG).show();
			setListAdapter(new ArrayAdapter<String>(this, R.layout.list, result));
		}
		else if(category.equalsIgnoreCase("Reviews")){
			String result[] = findReviews();
			Toast.makeText(getApplicationContext(), "Number of reviews found: " + result.length, Toast.LENGTH_LONG).show();
			setListAdapter(new ArrayAdapter<String>(this, R.layout.list, result));
		}
	...
	private String[] findPlaces() {
        ...
        try {
            ...
            Log.e("List Activity", "Exception finding " + this.what + " offers in " + this.zipCode );
            places = new String[] {"Exception finding " + this.what + " offers: " + e.getMessage()};
        }
        return places;		
	}
    ...
    private String[] findOffers() {
        ...
        try {
            ...
            Log.e("List Activity", "Exception findinOffersg " + this.what + " offers in " + this.zipCode );
            offers = new String[] {"Exception finding " + this.what + " offers: " + e.getMessage()};
        }
        return offers;		
	}
    ...
    private String[] findReviews() {
        ...
        search.setWhere(this.zipCode);
        if(sorted){
        	//Create date : CGReviewsSearchSort.CreateDate
        	//reviewRating : CGReviewsSearchSort.ReviewRating
        	search.setSort(CGReviewsSearchSort.CreateDate);
        }
        else{
        	search.setSort(CGReviewsSearchSort.Unknown);
        }
        ...
        try {
            ...
            Log.e("List Activity", "Exception finding " + this.what + " reviews in " + this.zipCode );
            reviews = new String[] {"Exception finding " + this.what + " reviews: " + e.getMessage()};
        }
        return reviews;		
	}
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
        ...
        if(category.equalsIgnoreCase("Places")){
        	Toast.makeText(getApplicationContext(), "Getting details of your place, sit tight...",
			...
			Toast.makeText(getApplicationContext(), "Getting details of your offer, sit tight...",
                    Toast.LENGTH_SHORT).show();
			...
			Toast.makeText(getApplicationContext(), "Getting details of your review, sit tight...",
                    Toast.LENGTH_SHORT).show();
			CGReviewsSearchReview review = cgReviews[position];
			StringBuilder reviewDetail = new StringBuilder();
			reviewDetail.append("Title : ").append(review.getTitle()).append("\n");
			reviewDetail.append("Author : ").append(review.getAuthor()).append("\n");
			reviewDetail.append("Rating : ").append(review.getRating()).append("\n\n");
